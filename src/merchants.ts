// The mock-merchant registry — merchant IDENTITY only: a stable ES256 signing key
// (DEV/THROWAWAY — testnet/mock posture, never real value), a byte-stable terms
// document (with an illustrative AAA dispute clause), and a dispute-resolution
// declaration. Products live in the ProductStore (src/lib/catalog.ts), seeded per
// merchant, so a catalog can be set up / edited at runtime.
//
...
export interface Merchant {
  ...
  payTo: string; // mock EVM address that receives x402 (USDC) payments — throwaway
}
